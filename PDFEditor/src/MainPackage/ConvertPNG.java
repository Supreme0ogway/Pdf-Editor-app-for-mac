...
public class ConvertPNG {
  ...
  public ConvertPNG(Stage primaryStage, Group root) {
      this.primaryStage = primaryStage;
      this.root = root;
  }

  public void start() {
      primaryStage.setTitle("Convert PDF to PNG");
    ...
    //combine button
      ...
      combineButton.setOnAction(event -> {
          ...
          if(!filePaths.isEmpty() && filePaths.size() == 1) {
        	  ConvertToPngProcess convert = new ConvertToPngProcess("", "");
        	  convert.setPdfFilePath(filePaths.get(0));
        	  
        	  //get the file name
        	  String fileName;
        	  if(fileNameTextField.getText().isEmpty()) {
        		  fileName = "PDFtoPNG.png";
        	  } else {
        		  fileName = fileNameTextField.getText() + ".png";
        	  }
