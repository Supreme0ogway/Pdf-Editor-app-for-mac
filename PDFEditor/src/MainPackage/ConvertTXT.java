...
public class ConvertTXT {
  ...
  public ConvertTXT(Stage primaryStage, Group root) {
      this.primaryStage = primaryStage;
      this.root = root;
  }

  public void start() {
      primaryStage.setTitle("Convert PDF to txt");
    ...
    //combine button
      ...
      combineButton.setOnAction(event -> {
          ...
          if(!filePaths.isEmpty() && filePaths.size() == 1) {
        	  ConvertToTextProcess convert = new ConvertToTextProcess();
        	  convert.setPdfFilePath(filePaths.get(0));
        	  
        	  //get the file name
        	  String fileName;
        	  if(fileNameTextField.getText().isEmpty()) {
        		  fileName = "PDFtoText.txt";
        	  } else {
        	  ...
        	  convert.setTxtFilePath(wordFilePath);
