...
public class ConvertWORD {
  ...
  public ConvertWORD(Stage primaryStage, Group root) {
      this.primaryStage = primaryStage;
      this.root = root;
  }

  public void start() {
      primaryStage.setTitle("Convert PDF to Word (docx)");
    ...
    //combine button
      ...
      combineButton.setOnAction(event -> {
          ...
          if(!filePaths.isEmpty() && filePaths.size() == 1) {
        	  ConvertToWordProcess convert = new ConvertToWordProcess();
        	  convert.setReadingPath(filePaths.get(0));
        	  
        	  //get the file name
        	  String fileName;
        	  if(fileNameTextField.getText().isEmpty()) {
        		  fileName = "PDFtoWord.docx";
        	  } else {
        		  fileName = fileNameTextField.getText() + ".docx";
        	  }
        	  ...
        	  convert.setWritingPath(wordFilePath);
