...
public class DeletePDFpages {
  ...
  public DeletePDFpages(Stage primaryStage, Group root) {
  ...
  public void start() {
      ...
      // Message
      Text message = new Text("Delete PDF pages in range");
      ...
      //from___ to ____
      TextField Page1Swap = new TextField();
      Page1Swap.setPromptText("Page number. (Includes page)");
      Page1Swap.setTranslateX(320);
      Page1Swap.setTranslateY(286);
      Page1Swap.setPrefWidth(180);
      
      //from___ to ____
      TextField Page2Swap = new TextField();
      Page2Swap.setPromptText("Page number. (Includes page)");
      Page2Swap.setTranslateX(320);
      Page2Swap.setTranslateY(320);
      Page2Swap.setPrefWidth(180);
   ...
   // Add dropped files to the VBox
    ...
    //combine button
      Button combineButton = new Button("Delete Pages");
      ...
      combineButton.setOnAction(event -> {
    	  if(Page1Swap.getText().isEmpty() || Page2Swap.getText().isEmpty() || 
    			  (!Page1Swap.getText().matches("\\d+")) ||
    			  (!Page2Swap.getText().matches("\\d+"))) {
    		  System.out.println("no page numbers or contains string not number");
    	  } else {
    		// Use the filePaths list for further processing (e.g., combining the files)
              ...
              if(!filePaths.isEmpty() && filePaths.size() == 1) {
            	  DeletePDFpagesProcess convert = new DeletePDFpagesProcess();
            	  convert.setPdfFilePath(filePaths.get(0));
            	  convert.setFirstPage(Page1Swap.getText());
            	  convert.setSecondPage(Page2Swap.getText());
            	  
            	  //get the file name
            	  String fileName;
            	  if(fileNameTextField.getText().isEmpty()) {
            		  fileName = "EditedPagesPDF.pdf";
            	  } else {
            	  ...
            	  convert.setNewPDFfilePath(wordFilePath);
            	  ...
            	  //checks if the file exists before clearing
                  File def = new File(wordFilePath);
                  if(def.exists()) {
      ...
      root.getChildren().addAll(backButton, message, 
    		  ...
    		  pdfExt, pdfno, Page1Swap, Page2Swap);
  }
